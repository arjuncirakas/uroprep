...
const PatientDetails = () => {
  ...
  const handleAddDischargeSummary = (e) => {
    ...
    // Create new discharge summary entry
    const newDischargeSummary = {
      ...
      medications: dischargeForm.medications.filter(med => med.name.trim() !== '').map(med => med.name), // Convert to string array format
      summary: dischargeForm.dischargeNotes, // Use discharge notes as summary
      status: 'Discharged',
      surgeon: 'Dr. Smith', // Default values - can be made dynamic later
      ward: 'Urology Ward',
      readmissionRisk: 'Low',
      ...
      admissionDate: new Date(new Date(dischargeForm.dischargeDate).getTime() - 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 3 days before discharge
      createdAt: new Date().toISOString()
    };

    // Add to state (in real app, this would be sent to backend)
  ...
  // Function to remove discharge summary (for future backend integration)
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-gray-600">Normal (≤6.0)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">Elevated (6.1-6.5)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="w-3 h-3 bg-red-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">High (&gt;6.5)</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      <p className="text-2xl font-bold text-green-600">
                        {mockPatient.psaHistory.filter(psa => psa.value <= 6.0).length}
                      </p>
                      <p className="text-sm text-gray-600">Normal Tests</p>
                    </div>
                    <div className="text-center p-3 bg-yellow-50 rounded-lg">
                      <p className="text-2xl font-bold text-yellow-600">
                        {mockPatient.psaHistory.filter(psa => psa.value > 6.0 && psa.value <= 6.5).length}
                      </p>
                      <p className="text-sm text-gray-600">Elevated Tests</p>
                    </div>
                    <div className="text-center p-3 bg-red-50 rounded-lg">
                      <p className="text-2xl font-bold text-red-600">
                        {mockPatient.psaHistory.filter(psa => psa.value > 6.5).length}
                      </p>
                      <p className="text-sm text-gray-600">High Tests</p>
                    </div>
                    <div className="text-center p-3 bg-blue-50 rounded-lg">
                      <p className="text-2xl font-bold text-blue-600">
                        {(mockPatient.psaHistory[mockPatient.psaHistory.length - 1].value - mockPatient.psaHistory[0].value).toFixed(1)}
                      </p>
              ...
              <div className="flex justify-end space-x-3 mt-6 pt-6 border-t border-gray-200">
                ...
                <button
                  onClick={() => {
                    ...
                    navigate(`/urology-nurse/reschedule/${selectedAppointment.id}`);
                  }}
